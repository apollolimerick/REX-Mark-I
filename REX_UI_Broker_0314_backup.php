...
// Hierarchical Aggregate Indices (Global -> Continent -> Country -> City -> District)
...
// Establish universal Trade Target 
if ($view === 'terminal') {
    if ($node_type === 4 && isset($current_level_data['ticker'])) { // District is now level 4
...
if ($trade_target && $view === 'terminal') {
    ...
    // Generate 5 Years (1825 days) of backward continuous chart history
    $chartData = [];
    $current_c = $trade_target['price']; 
    for ($d = 0; $d <= 1825; $d++) {
        $date = date('Y-m-d', strtotime("-$d days"));
        
        $volatility = $current_c * 0.025; // 2.5% max daily volatility
        ...
        // Array Unshift to keep chronological order (oldest -> newest)
        array_unshift($chartData, [
            'd' => $date, 
            'o' => round($o,2), 'h' => round($h,2), 'l' => round($l,2), 'c' => round($current_c,2)
        ]);
        
        // Prepare for the "previous" day backward calculation
        $current_c = $o; 
    }
}
...
if ($view === 'portfolio') {
    ...
    // Generate backwards portfolio history so it connects smoothly to today
    $port_chart_data = [];
    ...
    $current_portfolio_value = $port_chart_data[1825]['c']; // Target today's value
    ...
    $portfolio_holdings = [
        ['date' => date('M d, Y', strtotime('-120 days')), 'ticker' => $flat_tickers_port[0], 'shares' => 15.5, 'avg_price' => 12500, 'current_price' => 13200, 'status' => 'Liquid'],
        ['date' => date('M d, Y', strtotime('-85 days')), 'ticker' => $flat_tickers_port[1], 'shares' => 42.0, 'avg_price' => 5400, 'current_price' => 4900, 'status' => 'Locked (30d)'],
        ['date' => date('M d, Y', strtotime('-45 days')), 'ticker' => $flat_tickers_port[2], 'shares' => 8.25, 'avg_price' => 22000, 'current_price' => 24500, 'status' => 'Liquid'],
        ['date' => date('M d, Y', strtotime('-15 days')), 'ticker' => $flat_tickers_port[3], 'shares' => 110.0, 'avg_price' => 1100, 'current_price' => 1250, 'status' => 'Locked (1Yr)'],
    ];
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Quick Copy Convenience Feature */
        ...
        .toast-notification {
            ...
            z-index: 9999; pointer-events: none; display: flex; align-items: center; gap: 10px;
        }
...
<body>
    ...
    <header class="fixed top-0 w-full z-50 p-5 flex justify-between items-center bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border-b border-white/40 dark:border-slate-700/50 transition-colors shadow-sm">
        ...
        <div class="flex items-center gap-5 relative z-10">
            ...
            <div class="h-6 w-px bg-[var(--border)] mx-1 hidden sm:block"></div>
            <button class="hidden sm:block text-sm font-bold text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors tracking-wide">Log In</button>
            <button class="hidden sm:block bg-slate-900 text-white dark:bg-white dark:text-slate-900 px-5 py-2.5 rounded-xl text-sm font-bold shadow-md hover:-translate-y-0.5 transition-transform tracking-wide">Sign Up</button>
        </div>
    </header>
    ...
    <main class="pt-36 pb-24 px-6 sm:px-8 lg:px-12 max-w-[1500px] mx-auto relative z-10">
        ...
        <?php if ($view === 'terminal'): ?>
            ...
            <?php if ($node_type === 4 && $trade_target): ?>
            ...
            <?php else: ?>
                ...
                <div class="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div class="glass-pane px-10 py-6 inline-block w-full md:w-auto">
                        <div style="font-size: 36px; font-weight: 800; color: var(--text-main); font-family: 'Playfair Display', serif; letter-spacing: -0.5px;">
                            <?php 
                                ...
                                elseif($node_type === 3) echo "Districts in " . end($path);
                            ?>
                        </div>
                    </div>
                    ...
                    <?php if (isset($current_level_data['_meta'])): ?>
                        ...
                        ?>
                        <div class="glass-pane px-8 py-5 flex items-center justify-between gap-8 w-full md:w-auto">
                            ...
                            <div>
                                ...
                                <?php if($node_type < 3): ?>
                                    <?php 
                                        ...
                                        $child_count = count($value) - 1; 
                                        $label = ($node_type === 0) ? 'Countries' : (($node_type === 1) ? 'Cities' : 'Districts');
                                    ?>
            ...
            <?php if ($trade_target): ?>
                    ...
                    <div id="tab-vault" class="tab-content">
                        ...
                        <div class="form-group" style="margin-bottom: 40px;">
                            ...
                            <select class="w-full bg-[var(--bg-subtle)] border border-[var(--border)] text-[var(--text-main)] font-semibold p-4 rounded-xl outline-none focus:border-[var(--brand-color)] transition-all cursor-pointer">
                                ...
                                <option value="1m">1 Month Lockup (3.5% APY)</option>
                ...
                <div class="chart-panel" id="chartPanel">
                    <button class="btn-close" style="position: absolute; top: 24px; right: 24px;" onclick="closePanel()">×</button>
                                        ...
                                        tooltip: {
                                            ...
                                            callbacks: {
                                                label: function(context) {
                                                    // Handle array for candle (shows close) or single value for line
        ...
        <?php elseif ($view === 'offer'): ?>
            ...
            <div class="glass-pane max-w-4xl mx-auto p-12 sm:p-16 text-center mt-12">
                ...
                <button class="bg-slate-900 text-white dark:bg-white dark:text-slate-900 border-none px-10 py-5 rounded-2xl font-bold text-lg cursor-pointer transition-transform hover:-translate-y-1 shadow-[0_8px_24px_rgba(0,0,0,0.15)] dark:shadow-[0_8px_24px_rgba(255,255,255,0.15)]" onclick="window.location.href='REX_UI_Questionnaire.php'">Initialize Underwriting &rarr;</button>
            </div>
        ...
        <?php elseif ($view === 'portfolio'): ?>
            ...
            <div class="max-w-6xl mx-auto mt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                    ...
                    <div class="glass-pane p-8 flex flex-col justify-between">
                        <div>
                        ...
                        <button class="mt-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold py-2.5 px-4 rounded-xl hover:opacity-90 transition-opacity text-sm shadow-md border border-white/20">
                            + Deposit Funds
                        </button>
                    </div>
                </div>
                            ...
                            <table class="w-full text-left whitespace-nowrap">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="pb-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Status</th>
                                ...
                                <tbody class="divide-y divide-[var(--border)]">
                                    ...
                                    <tr>
                                        ...
                                        <td class="py-4">
                                            <?php if ($holding['status'] === 'Liquid'): ?>
                                                <span class="px-3 py-1 rounded-md text-xs font-bold bg-blue-500/10 text-blue-500 border border-blue-500/20">Liquid</span>
                                            <?php else: ?>
                                                <span class="px-3 py-1 rounded-md text-xs font-bold bg-purple-500/10 text-purple-500 border border-purple-500/20">🔒 <?= $holding['status'] ?></span>
                                            <?php endif; ?>
                                        </td>
        ...
        <?php elseif ($view === 'about'): ?>
            ...
            <!-- ABOUT REX VIEW -->
            <div class="glass-pane max-w-4xl mx-auto mt-12 overflow-hidden">
                <div class="p-12 sm:p-16 border-b border-[var(--border)] relative overflow-hidden">
                    ...
                    <h2 class="text-4xl font-serif font-bold text-slate-900 dark:text-white mb-6 relative z-10">About REX</h2>
                    <p class="text-xl text-slate-700 dark:text-slate-300 font-medium leading-relaxed max-w-2xl relative z-10">
                        Democratizing global real estate through algorithmic underwriting, decentralized liquidity, and fractionalized ownership.
                    </p>
                </div>
                <div class="p-12 sm:p-16 space-y-12">
                    <div>
                        <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-4">Our Vision</h3>
                        <p class="text-lg text-slate-600 dark:text-slate-400 leading-relaxed">
                            Historically, real estate has been the most illiquid and gatekept asset class on the planet. REX (Residential Property Exchange) was founded to break down these barriers. By leveraging advanced algorithmic valuation models and blockchain infrastructure, we allow homeowners to instantly tokenize their physical equity, while providing global investors frictionless access to hyper-local housing markets.
                        </p>
                    </div>
                    <div>
                        <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-4">The AMM Liquidity Engine</h3>
                        <p class="text-lg text-slate-600 dark:text-slate-400 leading-relaxed">
                            Unlike traditional REITs, REX operates on an Automated Market Maker (AMM) protocol. This ensures 24/7/365 liquidity across all our regional indices. When you trade on the terminal, you are interacting directly with decentralized liquidity pools, completely eliminating broker intermediaries, massive overhead fees, and lengthy settlement periods.
                        </p>
                    </div>
                    <div>
                        <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-6">Strategic Network Partners</h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                            <!-- Placeholder Partner Logos using CSS shapes -->
                            <div class="aspect-video bg-[var(--bg-subtle)] border border-[var(--border)] rounded-xl flex items-center justify-center p-4">
                                <div class="font-bold text-slate-500 uppercase tracking-widest text-xs text-center">Oracle<br>Network</div>
                            </div>
                            <div class="aspect-video bg-[var(--bg-subtle)] border border-[var(--border)] rounded-xl flex items-center justify-center p-4">
                                <div class="font-bold text-slate-500 uppercase tracking-widest text-xs text-center">Global<br>Appraisal API</div>
                            </div>
                            <div class="aspect-video bg-[var(--bg-subtle)] border border-[var(--border)] rounded-xl flex items-center justify-center p-4">
                                <div class="font-bold text-slate-500 uppercase tracking-widest text-xs text-center">Web3<br>Audit Firm</div>
                            </div>
                            <div class="aspect-video bg-[var(--bg-subtle)] border border-[var(--border)] rounded-xl flex items-center justify-center p-4">
                                <div class="font-bold text-slate-500 uppercase tracking-widest text-xs text-center">L1 Base<br>Protocol</div>
                            </div>
                        </div>
                    </div>
                </div>
